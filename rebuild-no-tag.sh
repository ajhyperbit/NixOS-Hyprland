...
# cd to config dir
#pushd ~/dotfiles/nixos/common/system
...
sudo nixos-rebuild switch -I nixos-config=/home/ajhyperbit/dotfiles/nixos/common/system/AJ-PC.nix --upgrade --show-trace &>nixos-switch.log || (cat nixos-switch.log | grep --color error && exit 1) || grep -P -n "dotfiles\/nixos\/([a-zA-Z]+).nix:[0-9]+:[0-9]+|\/home\/ajhyperbit\/dotfiles\/nixos\/([a-zA-Z]+).nix" nixos-switch.log | sed 's/:[[:blank:]]*/: /'
